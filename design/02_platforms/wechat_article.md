...
`social-wechat-article` 使用微信官方 API 端点，而不是浏览器自动化。

## 认证

`social-wechat-article` 的账号选择与使用体验与 OpenCLI profile 对齐。目标体验是用户通过 `opencli --profile <name> social-wechat-article ...` 选择公众号身份；插件在该 profile 下保存公众号 API 配置、secret 和 token cache。

当前 OpenCLI 版本的 root `--profile` 只传递给 Browser Bridge 命令。`social-wechat-article` 首版通过 `OPENCLI_PROFILE=<name>` 和 `opencli profile use <name>` 读取 OpenCLI profile；等 OpenCLI 上游将 root profile 传递给 `Strategy.LOCAL` 命令后，不需要改变 profile 存储模型。

插件不提供 `--account` 参数，也不读取旧式全局公众号凭据环境变量。每个 OpenCLI profile 对应一个 `social-wechat-article` 公众号配置。
...
`config.json` 保存 `platform`、`profile`、`display_name`、`app_id`、`app_id_masked`、`api_base` 和 `secret_ref`。`app-secret` 使用 profile 级本地 secret 文件保存，命令输出不得打印 AppSecret、access token 或完整认证 URL。

通过 appid/secret 获取的 access token 会缓存到当前 profile 的 `token.json`。
...
| 命令 | 访问级别 | 用途 |
|---|---|---|
| `auth-config` | write | 为当前 OpenCLI profile 配置公众号 AppID/AppSecret |
| `auth-status` | read | 读取当前 OpenCLI profile 的认证配置和 token cache 状态 |
| `profile-clear` | write | 清理当前 OpenCLI profile 的公众号配置和 token cache |
...
当前命令集中 Publishing 能力域已经可用。后续微信能力应继续放在同一个 `social-wechat-article` 命名空间下，例如已发布内容查询、评论管理和数据查询，而不是新增发布专用命名空间。

所有需要认证的命令都必须从当前 OpenCLI profile 解析目标公众号。写命令和 dry-run 输出应包含 `profile`、`account_name`、`account_id_masked` 以及远端 ID，便于审计。没有当前 profile、当前 profile 未配置 `social-wechat-article`、或 profile 配置平台不匹配时，写命令必须失败，不得猜测目标账号。
