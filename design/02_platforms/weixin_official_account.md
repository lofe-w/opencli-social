...
`social-weixin` 的账号选择与使用体验与 OpenCLI profile 对齐。目标体验是用户通过 `opencli --profile <name> social-weixin ...` 选择公众号身份；插件在该 profile 下保存公众号 API 配置、secret 和 token cache。

当前 OpenCLI 版本的 root `--profile` 只传递给 Browser Bridge 命令。`social-weixin` 首版通过 `OPENCLI_PROFILE=<name>` 和 `opencli profile use <name>` 读取 OpenCLI profile；等 OpenCLI 上游将 root profile 传递给 `Strategy.LOCAL` 命令后，不需要改变 profile 存储模型。

插件不提供 `--account` 参数，也不读取旧式全局公众号凭据环境变量。每个 OpenCLI profile 对应一个 `social-weixin` 公众号配置。
...
所有需要认证的命令都必须从当前 OpenCLI profile 解析目标公众号。写命令和 dry-run 输出应包含 `profile`、`account_name`、`account_id_masked` 以及远端 ID，便于审计。没有当前 profile、当前 profile 未配置 `social-weixin`、或 profile 配置平台不匹配时，写命令必须失败，不得猜测目标账号。
